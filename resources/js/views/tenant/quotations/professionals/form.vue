...
export default {
    ...
    props: ['showDialog','professional'],
    data() {
        return {
            titleDialog: 'Personal Médico',
    ...
    methods: {
        ...
        async submit() {
            this.profesional = this.appointment_professionals;
            await this.$http.post(`/${this.resource}/validate_professional`, this.profesional)
                .then(response => {
                    if (response.data.success) {
                        this.$emit('addProfessional', this.profesional);
                        this.close(false)
                    }
                })
                .catch(error => {
        ...
        close(flag) {
            if (flag) this.$emit('addProfessional', {});
            this.errors = {}
