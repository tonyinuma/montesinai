    ...
    <el-dialog :title="titleDialog" :visible="showDialog" @close="close" @open="create">
        <form autocomplete="off" @submit.prevent="submit">
            <div class="form-body">
                <div class="row">
                    <div class="col-md-4">
                        <div class="form-group" :class="{'has-danger': errors.medical_speciality_id}">
                            <label class="control-label">Especialidad</label>
                            <el-select v-model="form.medical_speciality_id" filterable
                                       popper-class="el-select-identity_document_type">
                                <el-option v-for="option in medical_specialities" :key="option.id" :value="option.id"
                                           :label="option.name"></el-option>
                            </el-select>
                            <small class="form-control-feedback" v-if="errors.medical_speciality_id"
                                   v-text="errors.medical_speciality_id[0]"></small>
                        </div>
                    </div>
                    <div class="col-md-4">
    ...
    async created() {
        await this.$http.get(`/${this.resource}/tables`)
            .then(response => {
                ...
                this.medical_specialities = response.data.medical_specialities
            })
        await this.initForm()
    },
    methods: {
        ...
        create() {
            this.titleDialog = (this.recordId) ? 'Editar Profesional Médico' : 'Nuevo Profesional Médico'
            if (this.recordId) {
