@php
    ...
    $appointment = \App\Models\Tenant\Appointment::where('quotation_id',$document->id)->first();
    $professionals = \App\Models\Tenant\AppointmentProfessional::where('appointment_id',$appointment->id)->get();
...
<table class="full-width mt-5">
    <tr>
        <td width="15%">Cliente/Doctor:</td>
        <td width="45%">{{ $customer->name }}</td>
    </tr>
    @foreach($professionals as $professional)
    <tr>
        @switch($professional->medical_professional->medical_speciality_id)
            @case(1)
            <td width="25%">ENFERMERO/A :</td>
            @break
            @case(2)
            <td width="25%">ANESTECIOLOGO/A :</td>
            @break
            @case(3)
            <td width="25%">TERAPEUTA :</td>
            @break
            @case(4)
            <td width="25%">PSICOLOGO/A :</td>
            @break
        @endswitch
        <td width="25%">{{ $professional->medical_professional->name }}</td>
    </tr>
    @endforeach
    <tr style="border-top: 1px solid">
        ...
        <td width="15%">{{ $appointment->admission_date }}</td>
    </tr>
{{--    <tr>--}}
{{--        <td>{{ $appointment->patient->identity_document_type->description }}:</td>--}}
{{--        <td>{{ $appointment->patient->document_number }}</td>--}}
{{--    </tr>--}}
    ...
    <tr>
        ...
        @if($appointment->admission_date != $appointment->departure_date)
            <td width="25%">Fecha de Fin:</td>
            <td width="15%">{{ $appointment->departure_date }}</td>
        @endif
    </tr>
    ...
    @if ($document->payment_method_type)
        <tr>
            ...
            <td width="25%">Hora de Fin:</td>
            <td width="15%">{{ \Carbon\Carbon::parse($appointment->departure_time)->format('h:i A') }}</td>
        </tr>
    @endif
    <tr>
        <td class="align-top">T. Habitacion:</td>
        <td width="25%">{{ $appointment->room_types->description }}</td>
    </tr>
</table>
